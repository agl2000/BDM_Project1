...
path = input("Enter the path of the files to be loaded to HBase: ") # Ask for the directory path
...
for file in os.listdir(path): # Scan all files in the directory to loop trought them
    ...
    if file.endswith(".csv"): # Check if the file is a CSV
        table_name = 'csv_table_' + file.replace('.csv', '').replace(' ', '_').lower() #Create a new table name for the CSV file
        ...
        table_name = 'json_table_' + file.replace('.json', '').replace(' ', '_').lower() #Create a new table name for the json file
